    ...
    <div className="flex-1 bg-slate-50 font-sans w-full">
      ...
      {cajasNormales.length > 0 && (
        <div className="w-full bg-white rounded-none sm:rounded-3xl shadow-sm border-t sm:border border-slate-200 mt-2">
          {/* Eliminamos el div con overflow-x-auto para que el sticky vertical funcione nativamente */}
          <table className="border-separate border-spacing-0 min-w-max w-full">
            <thead className="relative z-30">
              <tr>
                {/* ESQUINA HORARIO: top-[60px] (Para quedar debajo de días) y left-0. Quitamos el mt-4 que causaba el espacio blanco */}
                <th className="sticky left-0 top-[60px] z-40 bg-slate-100 border-b border-r border-slate-200 p-2 sm:p-3 w-[70px] sm:w-[110px] shadow-[2px_2px_5px_-2px_rgba(0,0,0,0.1)] align-middle">
                  <div className="flex items-center justify-center gap-1.5 text-slate-500 font-black text-xs uppercase tracking-wider h-full min-h-[40px]">
            ...
            <tbody>
              {horariosUnicos.map((horario, rowIndex) => (
                <tr key={horario} className={`group hover:bg-slate-50/50 transition-colors ${rowIndex % 2 === 0 ? 'bg-slate-50/30' : 'bg-white'}`}>
                  ...
                  {/* CELDAS DE HORARIOS VERTICALES: z-20 y sticky a la izquierda */}
                  <td className="sticky left-0 z-20 bg-slate-50/90 backdrop-blur-sm border-b border-r border-slate-200 p-3 w-[80px] sm:w-[120px] text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] align-top">
                    <span className="font-black text-slate-700 text-xs sm:text-base block mt-2">{horario}</span>
                  </td>
