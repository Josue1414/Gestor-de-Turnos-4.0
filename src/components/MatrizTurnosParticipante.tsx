          ...
          {diaActual.horariosMaestros.map((horario, rowIdx) => (
            <tr key={rowIdx}>
              ...
              {diaActual.cajas.map(caja => {
                ...
                if (esEspecial && rowIdx !== 0) return null; 

                if (esEspecial) {
                  return (
                    <td key={caja.id} rowSpan={diaActual.horariosMaestros.length} className="p-3 border-l border-indigo-100 bg-indigo-50/20 align-top">
                ...
                return (
                  <td key={caja.id} className="p-2 border-b border-l border-slate-200 bg-white">
                    {turno?.participanteId === 'CERRADO' ? (
                      <div className="h-16 bg-red-50 border-2 border-dashed border-red-100 rounded-xl flex flex-col items-center justify-center text-red-300">
                        <Lock size={16} />
                      </div>
