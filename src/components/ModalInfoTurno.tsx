...
const ModalInfoTurno: React.FC<ModalInfoTurnoProps> = ({
  ...
  // Envolvemos el JSX en createPortal apuntando a document.body
  return createPortal(
    <div 
      ...
      >
        <div className="bg-gradient-to-br from-indigo-600 to-blue-700 p-6 text-white relative">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black leading-tight truncate pr-4">{p?.nombre || 'Participante'}</h2>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <button 
              onClick={() => setCajaEntregada(!cajaEntregada)}
              className={`w-full flex items-center gap-3 p-3.5 rounded-2xl border-2 transition-all ${cajaEntregada ? 'border-blue-500 bg-blue-50/50' : 'border-slate-100 bg-white hover:border-slate-200 hover:shadow-sm'}`}
            >
              <div className={`p-1 rounded-md transition-colors ${cajaEntregada ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-300'}`}>
                <Check size={16} strokeWidth={3} />
              </div>
              <span className={`font-bold text-sm ${cajaEntregada ? 'text-blue-900' : 'text-slate-600'}`}>
                Caja entregada al participante
              </span>
            </button>

            <button 
              onClick={() => setCajaDevuelta(!cajaDevuelta)}
              className={`w-full flex items-center gap-3 p-3.5 rounded-2xl border-2 transition-all ${cajaDevuelta ? 'border-emerald-500 bg-emerald-50/50' : 'border-slate-100 bg-white hover:border-slate-200 hover:shadow-sm'}`}
            >
              <div className={`p-1 rounded-md transition-colors ${cajaDevuelta ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-300'}`}>
                <Check size={16} strokeWidth={3} />
              </div>
              <span className={`font-bold text-sm ${cajaDevuelta ? 'text-emerald-900' : 'text-slate-600'}`}>
                Caja devuelta por el participante
              </span>
            </button>
          </div>
        </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3">
          <button 
            ...
            onClick={onRemove}
            className={`flex-1 flex flex-col items-center justify-center py-2.5 rounded-xl text-xs font-black transition-all shadow-sm border
              ${countdown > 0 
                ? 'bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed' 
                : 'bg-red-50 border-red-100 hover:bg-red-500 hover:text-white text-red-600'}`}
          >
            <Trash2 size={16} className="mb-0.5" />
            {countdown > 0 ? `Espera (${countdown})` : 'Quitar turno'}
          </button>
          
          <button 
            onClick={onSave}
            className="flex-[1.5] flex items-center justify-center gap-2 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-sm font-black shadow-md transition-transform hover:-translate-y-0.5"
          >
            Guardar cambios
          </button>
        </div>
    ...
    document.body // <-- Elemento destino del Portal
  );
};
